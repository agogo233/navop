...
impl HomePage {
    ...
    /// 打开 RocketMQ 连接页签(工作区聚合/单连接两种模式,对齐 MQTT 页签策略)
    #[allow(clippy::too_many_arguments)]
    pub(crate) fn open_rocketmq_tab_with_mode(
        &mut self,
        conn: StoredConnection,
        workspace: Option<Workspace>,
        mode: TabOpenMode,
        ...
        let (tab_id, connections, workspace_for_tab) = match open_mode {
            DatabaseOpenMode::Workspace if workspace_id.is_some() => {
                let mut connections: Vec<StoredConnection> = self
                    ...
                    .iter()
                    .filter(|connection| connection.workspace_id == workspace_id)
                    .filter(|connection| connection.connection_type == ConnectionType::Rocketmq)
                    .cloned()
                    .collect();
                crate::connection_sort::sort_connections(&mut connections, connection_sort_order);
                let tab_id = format!("workspace-rocketmq-tab-{}", workspace_id.unwrap_or(0));
                (tab_id, connections, workspace)
            }
            _ => {
                let conn_id = conn.id.unwrap_or(0);
                let tab_id = format!("rocketmq-{conn_id}");
                (tab_id, vec![conn.clone()], None)
            }
        };
        ...
        window.defer(cx, move |window, cx| {
            ...
            tab_container.update(cx, |tc, cx| {
                tc.activate_or_add_tab_lazy_with_mode(
                    ...
                    mode,
                    move |window, cx| {
                        let rocketmq_view = cx.new(|cx| {
                            rocketmq_view::RocketmqTabView::new_with_active_conn(
                                workspace_for_tab,
                                connections,
                                active_conn_id,
                                window,
                        ...
                        TabItem::new(tab_id_for_tab, "rocketmq", rocketmq_view)
                    },
                    window,
                    cx,
                );
