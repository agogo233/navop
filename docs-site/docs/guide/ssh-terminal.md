...
![Agent Hub 工作区](/images/agent_hub.png)
...
2. **客户端安装 X11 服务**：本机必须有正在运行的 X server —— Windows 安装 XMing，macOS 安装 XQuartz。
   ...
   - XQuartz：[https://www.xquartz.org](https://www.xquartz.org)，也可用 Homebrew 安装：`brew install --cask xquartz`
   - macOS 安装完成后执行 `xhost +local:` 授予本机访问授权。
