...
WASM 和其他受控运行时具有资源与超时限制。扩展任务失败时应检查输入、权限、日志和兼容版本，不要通过无限放宽资源限制掩盖问题。

## 卸载与问题报告

卸载前关闭依赖该扩展的连接、ACP 会话和远程桌面。卸载会在后台执行并显示状态；完成前不要重复触发安装、重载或卸载。卸载会移除功能入口，但通常不会删除远端数据；已保存连接可能在重新安装兼容扩展后恢复可用。是否保留扩展设置取决于类型和版本。
