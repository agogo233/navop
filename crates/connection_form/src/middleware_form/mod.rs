...
//! 声明式 `DeclarativeFormTab`/`DeclarativeFormField` 配置驱动标签页渲染,
//! 固定区块(钥匙串/工作区/团队/云同步/SSH 隧道)由引擎统一处理。各中间件
//! (MQTT,后续 RocketMQ/Kafka 等)只需提供一个 `MiddlewareFormAdapter` 实现
//! 参数映射,并声明自己的标签页配置,即可复用整套页面与保存/测试流程。
...
pub use declarative::{notes_tab_group, ssh_tab_group};
